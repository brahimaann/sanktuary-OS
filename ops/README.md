...
4. Register the three scheduled tasks (PowerShell, as your user):
...
# Auto-deploy: every 2 minutes, starting at login
$a = New-ScheduledTaskAction -Execute 'conhost.exe' -Argument '--headless powershell.exe -NoProfile -ExecutionPolicy Bypass -File "C:\Users\brahi\Desktop\sanktuary-OS\ops\deploy.ps1"'
$t1 = New-ScheduledTaskTrigger -AtLogOn -User $env:USERNAME
$t2 = New-ScheduledTaskTrigger -Once -At (Get-Date).AddMinutes(1) -RepetitionInterval (New-TimeSpan -Minutes 2)
$t1.Repetition = $t2.Repetition
$s = New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries -MultipleInstances IgnoreNew -ExecutionTimeLimit (New-TimeSpan -Minutes 20)
Register-ScheduledTask -TaskName 'Sanktuary OS auto-deploy' -Action $a -Trigger @($t1, $t2) -Settings $s
```

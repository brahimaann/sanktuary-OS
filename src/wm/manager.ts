...
export type AppType = 'notepad' | 'calculator' | 'soundrec' | 'explorer' | 'iframe' | 'internet-explorer' | 'display-properties' | 'webradio' | 'pokemon' | 'pong' | 'africaonly' | 'video-folder' | 'video-player' | 'ppls-story' | 'ppls-thread-viewer' | 'ppls-local-echo' | 'winamp' | 'team-files' | 'file-preview' | 'network' | 'admin' | 'boards' | 'canvas' | 'teams' | 'chat' | 'profile' | 'planner';

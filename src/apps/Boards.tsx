...
const Boards: React.FC<{ kind?: 'canvas' | 'kanban' }> = ({ kind = 'canvas' }) => {
  ...
  const create = async () => {
    const name = (
      await dialog.prompt(plans ? 'Name the new plan (e.g. Album rollout):' : 'Name the new board:', '', {
        title: plans ? 'New Plan' : 'New Board',
      })
    )?.trim();
    if (!name) return;
    try {
      open(await api('/api/boards', { method: 'POST', body: JSON.stringify({ name, kind }) }));
      load();
    } catch (e) {
  ...
  const rename = async () => {
    ...
    if (!pick || !name) return;
    try {
      await api(`/api/boards/${pick.id}`, { method: 'PATCH', body: JSON.stringify({ name }) });
      load();
    } catch (e) {
      setStatus((e as Error).message);
    }
  ...
  const saveSharing = async (members: string[] | null) => {
    ...
    if (!pick) return;
    try {
      await api(`/api/boards/${pick.id}`, { method: 'PATCH', body: JSON.stringify({ members }) });
      load();
    } catch (e) {
      setStatus((e as Error).message);
    }
  ...
  const remove = async () => {
    if (
      !pick ||
      !(await dialog.confirm(`Delete the board "${pick.name}"?\nAn admin can still recover it from data\\boards-trash.`, {
        icon: 'warning',
      }))
    )
      return;
    try {
      await api(`/api/boards/${pick.id}`, { method: 'DELETE' });
      setSelected(null);
      load();
    } catch (e) {
      setStatus((e as Error).message);
    ...
    <div style={{ ...shell, position: 'relative' }}>
      <div style={toolbar}>
        <button style={button} onClick={create}>
          {plans ? 'New Plan...' : 'New Board...'}
        </button>
        <button style={button} disabled={!pick} onClick={() => pick && open(pick)}>
          Open
        </button>
        <button style={button} disabled={!pick} onClick={rename}>
          Rename
        </button>
        <button style={button} disabled={!pick} onClick={remove}>
          Delete
        </button>
        <button style={button} disabled={!canShare} onClick={() => setSharing(true)} title="Choose who can see it (creator or admin)">
          Sharing...
        </button>
      ...
      >
        {boards?.length === 0 && (
          <div style={{ padding: 8, gridColumn: '1 / -1' }}>
            Nothing here yet. Click <b>{plans ? 'New Plan...' : 'New Board...'}</b> to start one.
          </div>
        )}
        {boards?.map((b) => (
          <div
            ...
            style={{
              ...
              cursor: 'default',
              userSelect: 'none',
              background: selected === b.id ? '#000080' : undefined,
              color: selected === b.id ? '#fff' : undefined,
            }}
          >
            ...
            <div style={{ fontSize: 10, opacity: 0.8 }}>
              {b.items} item(s){b.online ? ` · ${b.online} online` : ''}
            </div>
            <div style={{ fontSize: 10, opacity: 0.8 }}>
              {new Date(b.updated).toLocaleDateString()} by {b.updatedBy}
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={statusBar}>
        {status ||
          `${boards?.length ?? 0} ${plans ? 'plan' : 'board'}(s) — ${isTouch ? 'tap' : 'double-click'} to open · drag one into a chat to share it`}
      </div>
    </div>
  );
};

...
const AfricaOnlyTV: React.FC = () => {
  ...
  return (
    <div style={shell}>
      ...
      <div style={toolbar}>
        {/* Nav buttons */}
        <button style={navBtn} onClick={goBack} disabled={histIdx === 0} title="Back">
          ◀
        </button>
        <button style={navBtn} onClick={goForward} disabled={histIdx >= history.length - 1} title="Forward">
          ▶
        </button>
        <button style={navBtn} onClick={refresh} title="Refresh">
          ↻
        </button>
        <button style={navBtn} onClick={() => navigate(TARGET_URL)} title="Home">
          🏠
        </button>
        ...
        <div style={addrBox}>
          ...
          <input
            ...
            spellCheck={false}
          />
          <button style={goBtn} onClick={() => navigate(address)}>
            Go
          </button>
        </div>
      ...
      <div style={{ flex: 1, position: 'relative', overflow: 'hidden', background: '#fff' }}>
        ...
        {blocked && (
          <div style={fallback}>
            ...
            <div style={{ fontWeight: 'bold', marginTop: 12, fontSize: 14 }}>This page cannot be displayed</div>
            <div style={{ color: '#555', fontSize: 11, marginTop: 6, maxWidth: 340, textAlign: 'center', lineHeight: 1.5 }}>
              <strong>AfricaOnly.TV</strong> has security settings that prevent it from being embedded inside another window
              (X‑Frame‑Options).
            </div>
            ...
            <div style={{ marginTop: 20, color: '#888', fontSize: 10 }}>{currentUrl}</div>
          </div>
        )}
      </div>
      ...
      <div style={statusBar}>
        ...
        <div style={{ ...statusSegment, width: 80, textAlign: 'center', borderLeft: '1px solid #808080' }}>Internet</div>
      </div>
    </div>
  );
};

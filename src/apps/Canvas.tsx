...
interface Item {
  ...
  url?: string;
  from?: string;
  to?: string; // edge: arrow from one item to another (x/y/w/h unused)
}
interface View {
  x: number;
  y: number;
...
const Board: React.FC<{ boardId: string }> = ({ boardId }) => {
  ...
  const { items, peers, status, setStatus, me, queue, flush, sendCursor, undo, redo, remember, snapshot, canUndo, canRedo } =
    useBoard<Item>(boardId);
  const [linking, setLinking] = useState<{ from: string; x: number; y: number } | null>(null);
  const [view, setView] = useState<View>(() => {
    try {
      return JSON.parse(localStorage.getItem(`sk_board_view_${boardId}`) || '') as View;
    } catch {
      return { x: 0, y: 0, z: 1 };
    }
  });
  ...
  useEffect(() => {
    try {
      localStorage.setItem(`sk_board_view_${boardId}`, JSON.stringify(view));
    } catch {}
  }, [boardId, view]);
  ...
  const drag = (e: React.PointerEvent, onMove: (dx: number, dy: number, ev: PointerEvent) => void, onEnd?: () => void) => {
    ...
    const move = (ev: PointerEvent) => {
      if (ev.pointerId === e.pointerId) onMove(ev.clientX - sx, ev.clientY - sy, ev);
    };
  ...
  const onBackgroundDown = (e: React.PointerEvent) => {
    ...
    drag(
      e,
      (dx, dy) => {
        if (pointers.current.size === 1) setView({ ...start, x: start.x + dx, y: start.y + dy });
      },
      () => pointers.current.delete(e.pointerId),
    );
  };
  ...
  const onLinkDown = (e: React.PointerEvent, item: Item) => {
    ...
    drag(
      e,
      (_dx, _dy, ev) => {
        end = toWorld(ev.clientX, ev.clientY);
        setLinking({ from: item.id, ...end });
      },
      () => {
        setLinking(null);
        ...
        if (
          hit &&
          hit.id !== item.id &&
          !Object.values(itemsRef.current).some((x) => x.type === 'edge' && x.from === item.id && x.to === hit.id)
        ) {
  ...
  const itemAt = (x: number, y: number) =>
    Object.values(itemsRef.current)
      .filter((i) => i.type !== 'edge' && x >= i.x && x <= i.x + i.w && y >= i.y && y <= i.y + i.h)
      .sort((a, b) => b.z - a.z)[0];
    ...
    for (const file of files) {
      try {
        ...
        if (fileKind(file.name) === 'image')
          add({ type: 'image', ...pos, ...(await imageSize(asset.src)), src: asset.src, name: asset.name });
  ...
  const onDrop = async (e: React.DragEvent) => {
    ...
    const teamFile = e.dataTransfer.getData(DRAG_FILE);
    if (teamFile) {
  ...
  const openItem = (item: Item) => {
    ...
    if (item.type === 'link') return /^https?:\/\//i.test(item.url || '') && window.open(item.url, '_blank', 'noopener');
    if (item.ref) {
      return openWindow({
    ...
    if (item.src?.startsWith('/api/')) window.open(item.src, '_blank');
  };
  ...
  const fit = () => {
    ...
    const minX = Math.min(...list.map((i) => i.x)),
      minY = Math.min(...list.map((i) => i.y));
    const maxX = Math.max(...list.map((i) => i.x + i.w)),
      maxY = Math.max(...list.map((i) => i.y + i.h));
  ...
  return (
    <div style={shell}>
      <div style={toolbar}>
        <button style={button} onClick={() => add({ type: 'note', ...center(), w: 200, h: 140, color: NOTE_COLORS[0], text: '' })}>
          Note
        </button>
        ...
        <button
          style={button}
          onClick={async () => {
            const u = (await dialog.prompt('Link URL:', 'https://', { title: 'Add link' }))?.trim();
            if (u && u !== 'https://') addLink(u);
          }}
        >
          Link...
        </button>
        ...
        />
        {one && (one.type === 'note' || one.type === 'text') && (
          <button style={button} onClick={() => setEditing(one.id)}>
            Edit
          </button>
        )}
        {one?.type === 'note' &&
          NOTE_COLORS.map((c) => (
            <button
              key={c}
              title="Note color"
              onClick={() => queue({ [one.id]: { ...one, color: c } }, true)}
              style={{ width: 16, height: 16, padding: 0, background: c, border: one.color === c ? '2px solid #000' : '1px solid #808080' }}
            />
          ))}
        {selected.size > 0 && (
          <button style={button} onClick={deleteSelected}>
            Delete
          </button>
        )}
        <button style={button} disabled={!canUndo} onClick={undo} title="Undo (Ctrl+Z)">
          ↶
        </button>
        <button style={button} disabled={!canRedo} onClick={redo} title="Redo (Ctrl+Shift+Z)">
          ↷
        </button>
        <div style={{ flex: 1 }} />
        <button style={button} onClick={() => zoomBy(1 / 1.25)}>
          −
        </button>
        <span style={{ minWidth: 38, textAlign: 'center' }}>{Math.round(view.z * 100)}%</span>
        <button style={button} onClick={() => zoomBy(1.25)}>
          +
        </button>
        <button style={button} onClick={fit}>
          Fit
        </button>
      ...
      <div
        ...
        onDoubleClick={(e) => {
          if (e.target === e.currentTarget || (e.target as HTMLElement).dataset.world)
            add({ type: 'note', ...toWorld(e.clientX, e.clientY), w: 200, h: 140, color: NOTE_COLORS[0], text: '' });
        }}
        onKeyDown={onKeyDown}
        onDragOver={(e) => {
          e.preventDefault();
      ...
      >
        ...
        >
          <Edges
            ...
            onSelect={(id, add) => {
              port.current?.focus();
              setSelected(add ? new Set(selected).add(id) : new Set([id]));
            }}
          />
          {Object.values(items)
              ...
              p.x !== undefined && (
                ...
                >
                  <svg width="14" height="20" viewBox="0 0 14 20">
                    <path d="M0 0 L0 16 L4 12 L7 19 L10 18 L7 11 L13 11 Z" fill={p.color} stroke="#fff" strokeWidth="1" />
                  </svg>
                  <span
                    style={{
                      background: p.color,
      ...
      <div style={statusBar}>
        {status ||
          `${Object.keys(items).length} item(s) · ${others.length ? `${others.length} other${others.length > 1 ? 's' : ''} here` : 'just you'} · drag background to pan · Ctrl+scroll or pinch to zoom · double-click to add a note`}
      </div>
    </div>
  );
};
...
const ItemView: React.FC<{
  ...
  onLink?: (e: React.PointerEvent) => void;
  onOpen: () => void;
  ...
  const box: React.CSSProperties = {
    ...
    outline: selected ? `${2 / zoom}px solid #000080` : 'none',
  ...
  const textStyle: React.CSSProperties = {
    width: '100%',
    height: '100%',
    padding: item.type === 'note' ? 12 : 0,
    boxSizing: 'border-box',
    border: 'none',
    outline: 'none',
    resize: 'none',
    background: 'transparent',
    font: item.type === 'text' ? '700 28px Arial, sans-serif' : '14px Arial, sans-serif',
    whiteSpace: 'pre-wrap',
    overflow: 'hidden',
    color: '#111',
  };
  ...
  if (item.type === 'note' || item.type === 'text') {
    body = editing ? (
      ...
      <div style={textStyle}>
        {item.text || <span style={{ color: '#999' }}>{item.type === 'note' ? 'Double-click to write' : 'Text'}</span>}
      </div>
    );
    if (item.type === 'note')
      body = (
        <div style={{ width: '100%', height: '100%', background: item.color || '#fff59d', boxShadow: '2px 3px 6px rgba(0,0,0,0.25)' }}>
          {body}
        </div>
      );
    ...
    body = (
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
          <img
            src={item.type === 'link' ? '/images/icons/internet-explorer-32x32.png' : fileIcon(item.name || '', false, 32)}
            alt=""
            style={{ width: 32, height: 32, flexShrink: 0 }}
          />
          <div style={{ minWidth: 0 }}>
            <div style={{ fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {item.type === 'link' ? item.text : item.name}
            </div>
            <div style={{ color: '#444', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {item.type === 'link'
                ? item.url
                : item.ref
                  ? `${item.ref.app}/${[...item.ref.dir, item.ref.name].join('/')}`
                  : 'attached file'}
            </div>
          </div>
        </div>
    ...
    >
      ...
      {selected && !editing && onLink && (
        <div
          ...
          title="Drag onto another card to connect them"
          style={{
            position: 'absolute',
            right: -22 / zoom,
            top: `calc(50% - ${8 / zoom}px)`,
            width: 16 / zoom,
            height: 16 / zoom,
            borderRadius: '50%',
            background: '#fff',
            border: `${2 / zoom}px solid #000080`,
            cursor: 'crosshair',
            boxSizing: 'border-box',
          }}
        />
      ...
      {lines.map((l) => (
        <g key={l.id}>
          <line
            ...
            x2={l.x2}
            y2={l.y2}
            stroke={l.sel ? '#000080' : '#333'}
            strokeWidth={l.sel ? w * 1.6 : w}
            markerEnd={`url(#sk-arrow-${l.sel ? '000080' : '333'})`}
          />
          <line
            x1={l.x1}
            y1={l.y1}
